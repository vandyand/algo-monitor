...
input int ma_shift4 = 0;

input int open_bars = 5;
input int iter = 0;
...
int OnInit(){
   ...
   maHandle1 = iMA(_Symbol,_Period,ma_period1,ma_shift1,MODE_SMA,PRICE_OPEN);
   maHandle2 = iMA(_Symbol,_Period,ma_period2,ma_shift2,MODE_SMA,PRICE_OPEN);
   maHandle3 = iMA(_Symbol,_Period,ma_period3,ma_shift3,MODE_SMA,PRICE_OPEN);
   maHandle4 = iMA(_Symbol,_Period,ma_period4,ma_shift4,MODE_SMA,PRICE_OPEN);
...
{   
   ...
   //Alert((long)TimeCurrent());
   Alert(GetMicrosecondCount());
   if(Find_New_Bar()){
      barNum++;
      ...
      if(MA1 > currentOpen && currentOpen > MA2){
         EntLRE0 = true;
      } else {
         EntLRE0 = false;
      }
      if(MA3 > currentOpen && currentOpen > MA4){
         EntSRE0 = true;
      } else {
         EntSRE0 = false;
      }
      ...
      if(barNum == closeBar){
         while(!CloseRecentPosition()&&order_count<5){Sleep(10);order_count++;}
         order_count = 0;
      }
      
      
      if(CurrentOpenPositions(MagicNumber) < 1){
         if(EnterLongSignal && !EnterShortSignal){
            while(!BuyAsync(Lots)&&order_count<5){Sleep(10);order_count++;}
            closeBar = barNum + open_bars;
            order_count = 0;
         }
         if(EnterShortSignal && !EnterLongSignal){
            while(!SellAsync(Lots)&&order_count<5){Sleep(10);order_count++;}
            closeBar = barNum + open_bars;
            order_count = 0;
         }
      }
...
//--- calculate the ratio of trend profits to the standard deviation
   ...
   int arg = ma_period1 + ma_period2 + ma_period3 + ma_period4 + ma_shift1 + ma_shift2 + ma_shift3 + ma_shift4;
