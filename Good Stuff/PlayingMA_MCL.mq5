...
double OnTester()  {
   ...
   if(useTargets && backProf > threshold_lower && foreProf > threshold_lower && verifyProf > threshold_lower
   ...
   rtn *= useTargets ? MathAbs(totalProf) * MathAbs(TesterStatistics(STAT_SHARPE_RATIO)) :
          totalProf * MathAbs(TesterStatistics(STAT_SHARPE_RATIO));
...
//--- calculate the ratio of trend profits to the standard deviation
   ...
   double st_dev = MathPow(sumsqrs,0.5);
	...
	double scaled_gross_profit = (gross_profit) / 20.0;
	scaled_gross_profit = scaled_gross_profit > 0 ? scaled_gross_profit < 10.0 ? scaled_gross_profit : 10.0 : 0;
	
	double scaled_sqn = (sqn + 5.0) / 1.0;
	scaled_sqn = scaled_sqn > 0 ? scaled_sqn < 10.0 ? scaled_sqn : 10.0 : 0;
	
	double scaled_drawdown = ((equity_dd_percent * -1.0) + 25.0) / 2.50;
	scaled_drawdown = scaled_drawdown > 0 ? scaled_drawdown < 10.0 ? scaled_drawdown : 10.0 : 0;
	
	double scaled_sharpe = (sharpe_ratio + 2.0)/0.4;
	scaled_sharpe = scaled_sharpe > 0 ? scaled_sharpe < 10.0 ? scaled_sharpe : 10.0 : 0;
	
	double scaled_expectancy = (expectancy + 7.5)/1.50;
	scaled_expectancy = scaled_expectancy > 0 ? scaled_expectancy < 10.0 ? scaled_expectancy : 10.0 : 0;
	
	double scaled_regression = (regression + 1000.0)/200.0;
	scaled_regression = scaled_regression > 0 ? scaled_regression < 10.0 ? scaled_regression : 10.0 : 0;

	double scaled_gross_loss = (gross_loss + 100.0)/10.0;
	scaled_gross_loss = scaled_gross_loss > 0 ? scaled_gross_loss < 10.0 ? scaled_gross_loss : 10.0 : 0;
	
	double useful_loss = gross_loss < 0 ? gross_loss > -50 ? 50 + gross_loss : 0 : 0;
	
	double rtn = scaled_gross_profit * scaled_sqn * scaled_sharpe + scaled_expectancy + scaled_regression + scaled_drawdown + scaled_gross_loss;
	//double rtn = 1.0;
   ...
   //double rtn = gross_loss > 0 ? regression * sqn * sharpe_ratio * profit * expected_payoff / gross_loss: 0;
    ...
    open_bars1 + open_bars2 + open_bars3 + open_bars4 + open_bars5 +
    open_bars6 + open_bars7 + open_bars8 + open_bars9 + open_bars10 +
    open_bars11 + open_bars12 + open_bars13 + open_bars14 + open_bars15 + open_bars16;
